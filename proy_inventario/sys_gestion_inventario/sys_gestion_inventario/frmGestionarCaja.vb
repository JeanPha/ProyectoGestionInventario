...
Public Class frmGestionarCaja
    Private Sub frmGestionarCaja_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Using transaction As SqlTransaction = conexion.getConexion().BeginTransaction()
                Try
                    ...
                    Dim queryAbrirCaja As String = "INSERT INTO abrirCaja (idCaja, idMetodo, monto, fecha, idEmpleado) VALUES (@idCaja, @idMetodo, @monto, @fecha, @idEmpleado)"
                    ...
                    transaction.Commit()
                    MessageBox.Show("Apertura de caja guardada correctamente.")
                Catch ex As Exception
    ...
    Private Function ObtenerIdCajaNuevo() As Integer
        ...
        Dim sql As String = "SELECT ISNULL(MAX(idCaja), 0) + 1 FROM abrirCaja"
